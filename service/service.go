...
var apiResponseTime = promauto.NewHistogramVec(prometheus.HistogramOpts{
	Name:    "api_response_time_seconds",
	Help:    "Histogram of response times for API in seconds",
	Buckets: prometheus.DefBuckets, // Вы можете настроить свои интервалы
...
func fetchCurrencyRates(baseCurrency string) (models.CurrencyRates, error) {
	...
	log.Info("Logging before observing the metric")

	apiResponseTime.WithLabelValues(url, statusCode).Observe(duration.Seconds())
	log.Info("Logging after observing the metric")
